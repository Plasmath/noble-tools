...
def maketets(l): #Generates all possible subsets of 4 vertices containing a given point in a list of points
    z = l[0]
    return [[z,l[i+1],l[j+2],k] for i in range(len(l)-2) for j in range(i,len(l)-3) for k in l[j+3:]]

def findvols(points,cap): #Finds all tetrahedron volumes given a set of points
